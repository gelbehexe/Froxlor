	...
	protected function getVhostContent($domain, $ssl_vhost = false)
	{
		...
		if (preg_match('/^https?\:\/\//', $domain['documentroot'])) {
			...
			if ($this->deactivated == false) {
                ...
                foreach($specialSettings as $type => $specialSettingsItem) {
                    if ($type === 'default_vhost_content' || empty($specialSettingsItem[0])) {
                        continue;
                    }

                    $vhost_content = $this->mergeVhostCustom($vhost_content, $specialSettingsItem[0]);
                }
